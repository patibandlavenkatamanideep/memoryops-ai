...
`{ total, passed, failed, pass_rate, results[] }`. Still `planned` for authorization —
gated today by `MEMORYOPS_PUBLIC_EVALS`; `evals:run` enforcement lands with the
governance mutations.

## GET /api/evals/latest
Requires `evals:read` (auditor / tenant admin). A **pure read** of the last completed
run — it never triggers one.
...
activity and require `traces:read:tenant`. A `loop_run_id` or `trace_id` is not an
authorization token: the tenant remains a predicate in the query, so a leaked id
returns nothing outside its tenant.

## GET /api/traces (v1.8)
Checks `traces:read:tenant` (auditor / tenant admin) as defence in depth, but is
listed **`planned`, not `enforced`**, in the authorization matrix.

> **The buffer is process-wide, not tenant-partitioned.** Spans carry no tenant id,
> user id, or memory content, so nothing confidential is disclosed — but a permitted
> caller can observe the timing, volume and decisions of operations generated by other
> tenants in the same process. Calling that `enforced` under a `:tenant` permission
> would claim a scope the runtime does not provide. Resolving it means either
> attaching a tenant to spans and filtering by it, or reclassifying this as
> deployment-level telemetry under a future `ops:traces` permission.
