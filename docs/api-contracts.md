...
Requires `evals:read` (auditor / tenant admin). Results are tenant-wide governance
evidence, so `memory_admin` does not hold it. Authorized **before** the cache is
consulted: a miss regenerates, and that is real compute an unauthorized caller must
...
## GET /api/traces (v1.8)
Requires `traces:read:tenant` (auditor / tenant admin). Spans describe *what the
system did and when*, which is governance evidence rather than memory access —
`memory_admin` deliberately does not hold it.
...
> caller can observe timing and volume for operations generated by other tenants in
> the same process. The permission is the control that exists today; partitioning is
> separate work.
