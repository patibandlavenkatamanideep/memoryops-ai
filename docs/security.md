...
`GET /api/evals/latest` regenerates its cache on a miss, so authorizing after the
cache lookup would let an unauthorized caller drive real evaluation compute — a
denial-of-wallet vector on a public deployment. It is checked first, with a test
asserting the harness is never reached, and a positive control asserting the
authorized read does reach it.

#### Limitation: `/api/traces` is process-wide

`traces:read:tenant` controls *who* may read spans, but the ring buffer is not
partitioned by tenant. Spans carry no tenant id, user id, or memory content (asserted
in tests), so nothing confidential is disclosed — but a permitted caller can observe
the timing and volume of operations generated by other tenants sharing the process.
Partitioning the buffer is separate work; the permission is the control that exists
today.
