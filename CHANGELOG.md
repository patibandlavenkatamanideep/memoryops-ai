...
**25 of 39 routes enforced** (was 10). Planned 5 — all governance *mutations* —
and 9 public.
...
- Authorization runs before any repository read, loop event, audit write, or
  computation. `GET /api/evals/latest` regenerates on a cache miss, so an
  unauthorized caller must never reach it — asserted, with a positive control that
  the authorized read does.
- Every route forces the query to `principal.tenant_id` after authorizing; the
  request's own value is not reused.

### Known limitation
- `GET /api/traces` is now permission-gated but the span buffer is **process-wide**,
  not tenant-partitioned. Spans carry no tenant, user, or memory content (asserted),
  so this discloses no data — but a permitted caller sees timing and volume for
  operations generated by other tenants in the same process. Partitioning the buffer
  is separate work.
